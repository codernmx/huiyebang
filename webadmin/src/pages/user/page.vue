		...
		<div>
			...
			<standard-table :columns="columns" :rowKey="'uid'" :pagination.sync="pagination" :dataSource="dataSource"
				...
				<div slot="description" slot-scope="{text, record}">
					<a-switch :default-checked="record.is_teacher==1" @change="switchChange(record)" />
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			switchChange(record) {
				if (record.is_teacher==1){
					record.is_teacher = 0
				}else{
					record.is_teacher = 1
				}
				console.log(record)
				this.updateUser({'uid':record.uid,'is_teacher':record.is_teacher})
			},
			init() {

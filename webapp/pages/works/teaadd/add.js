...
Page({
  ...
  //编辑学习作品时获取作品详情
  async getDetail(id){
    const res = await app.http('/public/compositionteacher/find','get',{cs_id:id});
    this.setData({
      formData:{
        title:res.data.name,
  ...
  async submit(){
    ...
    if(this.data.submitType == 'add'){
      await app.http('/public/compositionteacher/add','post',data);
    ...
    if(this.data.submitType == 'edit'){
      await app.http('/public/compositionteacher/update','post',{...data,ct_id:+this.data.id});
      app.toast({ title:'保存成功',icon:'success',callback:()=>{
         wx.navigateTo({
            url: '/pages/teacherWorksList/index',
        })
      } })
      return;
    }
